...
public class AccountStepDefinitions extends CucumberStepDefinition {
    ...
    @When("^client transfers \"(.*)\" to account  \"(.*)\"$")
    public void clientTransfersAmountToAccountAccountNr(String amount, String accountNr) {
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);
        headers.add("clientNr", clientNr);
        HttpEntity<String> entity = new HttpEntity<String>("parameters", headers);
        String url = this.rootUri + "/accounts/" + this.accountNr + "/transfer/" + amount+"/"+accountNr;
        response = template.exchange(url, HttpMethod.PUT, entity, Long.class);
    }
